...
1. intent_type: one of "discover", "checkout", "track", "support", "browse"
2. search_query: the product/category to search for (only for discover intent). Use 1-3 key terms. If unclear, use empty string.
3. entities: list of {type, value} e.g. [{"type":"location","value":"NYC"}]

Rules:
- "discover" = user wants to find/browse products
...
- Return valid JSON: {"intent_type":"...","search_query":"...","entities":[],"confidence_score":0.0-1.0}
"""
...
async def _resolve_via_azure(text: str) -> Dict[str, Any]:
    ...
    if intent_type not in ("discover", "checkout", "track", "support", "browse"):
...
def _heuristic_resolve(text: str) -> Dict[str, Any]:
    ...
    # Discover: use action-word stripping
    derived = derive_search_query(text)
    return {
        "intent_type": "discover",
